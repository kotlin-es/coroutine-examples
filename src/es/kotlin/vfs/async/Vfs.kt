...
abstract class Vfs {
	...
	open fun readFullyAsync(path: String): Promise<ByteArray> = async {
		val stat = await(statAsync(path))
		await(readChunkAsync(path, 0L, stat.size))
	}
	...
	open fun list(path: String): AsyncSequence<VfsStat> = throw UnsupportedOperationException()
	...
	abstract class Proxy : Vfs() {
		...
		override fun statAsync(path: String): Promise<VfsStat> = async { transformStat(await(access(path).statAsync())) }
